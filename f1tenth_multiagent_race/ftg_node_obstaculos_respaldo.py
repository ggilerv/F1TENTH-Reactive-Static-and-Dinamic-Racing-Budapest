#!/usr/bin/env python3
"""
Follow The Gap + Disparity Extender — respaldo de la variante para
obstáculos estáticos (versión conservadora, velocidad_max=8.0)
==========================================================================

Autor: George Gabriel Giler Vega

Copia de respaldo de `ftg_node_obstaculos.py`, congelada en la versión
validada sin choques en 3 minutos de prueba continua (velocidad_max=8.0
m/s). Se conserva sin cambios como referencia y como opción de vuelta
atrás si el ajuste de mayor velocidad de `ftg_node_obstaculos.py`
...
class FollowTheGapObstaculosRespaldo(Node):
    """Nodo ROS 2 Follow The Gap, reajustado para obstáculos estáticos pequeños (respaldo)."""
    ...
    def __init__(self):
        ...
        super().__init__('follow_the_gap_obstaculos_respaldo')

        self.declare_parameter('rango_maximo', 8.0)
    ...
    def _planificar_velocidad(self, steering_angle, distancia_frenado,
        ...
        adicional por proximidad bruta (`v_proximidad`): a diferencia de
        `distancia_frenado` (que solo mira un cono angosto alrededor del
        objetivo y del frente), este límite usa el punto más cercano de
        todo el escaneo, sin importar de qué lado esté. Es necesario
        porque un obstáculo pequeño a un costado de la pista puede
        quedar fuera del cono de frenado mientras el auto sigue
        acelerando, y solo lo detecta la burbuja de seguridad cuando ya
        está muy cerca; este límite obliga a frenar antes, en cuanto
        cualquier punto del entorno (esté donde esté) se acerca.
        """
        ...
        distancia_proximidad_segura = max(0.0, min_dist_bruto - self.RADIO_VEHICULO)
...
def main(args=None):
    ...
    nodo = FollowTheGapObstaculosRespaldo()
    try:
        rclpy.spin(nodo)
